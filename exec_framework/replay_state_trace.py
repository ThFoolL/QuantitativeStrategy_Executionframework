...
def trace_window(data_path: Path, start_ts: str, end_ts: str, strategy_adapter: str = DEFAULT_STRATEGY_ADAPTER) -> pd.DataFrame:
    ...
    for idx, bar_ts in enumerate(fast_df.index):
        ...
        rows.append({
            ...
            'entry_price': state.get('execution_entry_price') or state.get('strategy_entry_price'),
            'strategy_ref_entry_price': state.get('strategy_ref_entry_price'),
            'execution_entry_price': state.get('execution_entry_price'),
            'high_water_r': state.get('execution_high_water_r', state.get('high_water_r')),
            ...
            'risk_amount': state.get('execution_risk_amount') or state.get('risk_amount'),
