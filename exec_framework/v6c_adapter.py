...
class V6CLiveAdapter:
    ...
    """Early live adapter for v6c.

    Current stage:
    - supports basic trend entry
    - supports first-pass trend position management
    - supports basic reversion entry/management
    - supports basic conflict resolution / flip planning

    Boundary note:
    - Adapter emits local action intent plus a small state patch in `conflict_context`.
    - It does not prove exchange fill/position truth.
    - Real executor/reconcile layer must decide which fields become confirmed state.
    """
    ...
    def _with_management_stop_update_phase(
        self,
        *,
        base_ctx: dict[str, object],
        stop_price_before: float,
        stop_price_after: float,
    ) -> dict[str, object]:
        ctx = dict(base_ctx)
        if abs(stop_price_after - stop_price_before) > 1e-12:
            ctx['pending_execution_phase'] = 'management_stop_update_pending_protective'
        return ctx
    ...
    def _manage_trend_position(self, market: MarketSnapshot, state: LiveStateSnapshot) -> FinalActionPlan | None:
        ...
        if state.active_side == 'long':
            ...
            intrabar_high_r = (high - entry_price) / risk_per_unit
            high_water_r = max(state.high_water_r, current_r, intrabar_high_r)
            ...
            base_ctx = self._with_management_stop_update_phase(
                ...
                stop_price_before=stop_price,
                stop_price_after=next_stop_price,
            )
            ...
            # `state_update` here means local strategy-state progression only; it is not an
            # exchange-side event and should not be interpreted as a real order acknowledgement.
        ...
        intrabar_high_r = (entry_price - low) / risk_per_unit
        high_water_r = max(state.high_water_r, current_r, intrabar_high_r)
        elapsed_minutes = (pd.Timestamp(market.bar_ts) - pd.Timestamp(state.strategy_entry_time)).total_seconds() / 60.0 if state.strategy_entry_time else 0.0
        open_impulse_fail = (
            self._is_us_open_impulse(state.strategy_entry_time or market.bar_ts)
    ...
    def _plan_trend_entry(self, market: MarketSnapshot, state: LiveStateSnapshot) -> FinalActionPlan:
        ...
        long_expansion_bias = (trend_close > ema_fast > ema_slow and structure_tag == 'EXPANSION' and adx >= state.adx_long_threshold and atr_rank >= state.atr_rank_long_threshold and session_tag in {'US_CORE', 'NON_US_ACTIVE'})
        long_trend_cont_bias = (trend_close > ema_fast > ema_slow and structure_tag == 'TREND_CONT' and adx >= state.adx_trend_cont_long_threshold and atr_rank >= state.atr_rank_trend_cont_long_threshold and session_tag in {'US_CORE', 'NON_US_ACTIVE'})
        short_expansion_bias = (trend_close < ema_fast < ema_slow and structure_tag == 'EXPANSION' and adx >= state.adx_short_threshold and atr_rank >= state.atr_rank_short_threshold and session_tag in {'US_CORE', 'NON_US_ACTIVE'})
        short_trend_cont_bias = (trend_close < ema_fast < ema_slow and structure_tag == 'TREND_CONT' and adx >= state.adx_trend_cont_short_threshold and atr_rank >= state.atr_rank_trend_cont_short_threshold and session_tag in {'US_CORE', 'NON_US_ACTIVE'})
