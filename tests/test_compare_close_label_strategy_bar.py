...
class CompareCloseLabelStrategyBarCase(unittest.TestCase):
    def test_close_labeled_decision_maps_to_next_slot(self) -> None:
        decision_time = datetime(2026, 1, 5, 15, 59, 59, 999000, tzinfo=timezone.utc)
        strategy_bar = compare.strategy_bar_from_decision_time(decision_time, minutes=5)
        self.assertEqual(strategy_bar.isoformat(), '2026-01-05T16:00:00+00:00')

    def test_window_attribution_uses_correct_slot_for_close_labeled_open(self) -> None:
        ...
        records = [
            {
                ...
                'strategy_ts': compare.strategy_bar_from_decision_time(
                    datetime(2026, 1, 5, 15, 59, 59, 999000, tzinfo=timezone.utc),
                    minutes=5,
                ).isoformat(),
                'plan': {
                    'action_type': 'open',
