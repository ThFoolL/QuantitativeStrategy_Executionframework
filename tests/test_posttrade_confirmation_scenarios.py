...
SCENARIOS = [
    ...
    {
        'name': 'close_position_protection_confirmed_from_open_orders',
        'request': {
            'symbol': 'BTCUSDT',
            'side': 'SELL',
            'quantity': 0.0,
            'reduce_only': True,
            'client_order_id': 'sample-close-position-protect',
        },
        'receipt': {
            'client_order_id': 'sample-close-position-protect',
            'exchange_order_id': '91010',
            'acknowledged': True,
        },
        'order': {
            'status': 'NEW',
            ...
            'side': 'sell',
            'position_side': 'both',
            'update_time_ms': 1711380010000,
        },
        'trades': [],
        'open_orders': [
            {
                'orderId': '91010',
                'clientOrderId': 'sample-close-position-protect',
                'status': 'NEW',
                'type': 'STOP_MARKET',
                'timeInForce': 'GTC',
                'side': 'SELL',
            ...
            {
                'orderId': '91011',
                'clientOrderId': 'sample-close-position-protect-tp',
                'status': 'NEW',
        ...
        'expected': {
            ...
            'freeze_reason': None,
            'should_freeze': False,
            'notes_contains': ['no_matching_user_trades'],
...
class PostTradeConfirmationScenariosCase(unittest.TestCase):
    ...
    def make_request(self, payload: dict, metadata: dict | None = None) -> BinanceOrderRequest:
        ...
        for scenario in self.scenarios:
            with self.subTest(name=scenario['name']):
                ...
                confirmation = confirmer.confirm(
                    ...
                    order_requests=[self.make_request(scenario['request'], scenario.get('request_metadata'))],
                ...
                if 'order_status' in expected:
                    self.assertEqual(confirmation.order_status, expected['order_status'])
